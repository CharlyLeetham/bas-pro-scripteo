...
//			var_dump(stripslashes( wp_filter_post_kses( addslashes( $_POST["html"] ) ) ));
			...
			if ( $capping == 0 ) {
				$_SESSION['bsa_capped_ads_' . bsa_ad($_GET['ad_id'], 'space_id')] = str_replace(',' . $_GET['ad_id'], '', $_SESSION['bsa_capped_ads_' . bsa_ad($_GET['ad_id'], 'space_id')]);
		...
		if ( isset($_POST["buyer_email"]) && $_POST["buyer_email"] != '' && isset($_POST["space_id"]) && $_POST["space_id"] != '' && isset($_POST["ad_model"]) && $_POST["ad_model"] != '' && isset($_POST["ad_limit_" . $_POST["ad_model"]]) && $_POST["ad_limit_" . $_POST["ad_model"]] != '' ) {
			...
			$model->addNewAd(	NULL, $_POST["space_id"], $ad_name, $_POST["buyer_email"], $_POST["title"], $_POST["description"], $_POST["button"], $_POST["url"], $fileName, stripslashes( $_POST["html"] ), $optional_field, $_POST["ad_model"], $ad_limit, $capping, 0.00, 2, $status); // paid 2 - Added via Admin Panel
...
/* -- END -- TEMPLATE */';
			// file_exists(plugin_dir_path( __DIR__ ) . 'frontend/css/block-'.$ad_width.'--'.$ad_height.'.css')
